...
class TemperatureMachine(StateMachine):
    ...
    ##
    ## on_cool_pulse - Action performed when the state machine transitions
    ## into the 'cool' state and over setPoint temp
    ##
    def on_cool_pulse(self):
        ##pulses red light 
        blueLight.pulse()

        if(DEBUG):
            print("* cooling to temp")
    
    ##
    ## on_heat_pulse - Action performed when the state machine transitions
    ## into the 'heat' state and under setPoint temp
    ##
    def on_heat_pulse(self):
        ##pulses red light 
        redLight.pulse()
    ...
    def on_enter_off(self):
        #turn off both lights because system is off
    ...
    def processTempStateButton(self):
        ...
        #sends cycle events to the statemachine that is triggered by button_press 
        self.send("cycle")
    ...
    def processTempIncButton(self):
        ...
        #increases the set point by one then updates the light
        self.setPoint += 1
        self.updateLights()
    ...
    def processTempDecButton(self):
        ...
        #decreases the setPoint by one and updates the lights accordingly
        self.setPoint -= 1
        self.updateLights()
    ...
    def updateLights(self):
        ...
        #if state is off use on_enter_off to turn off the lights
        if(self.current_state.id == 'off'):
            self.on_enter_off()

        #if state is HEAT
        elif(self.current_state.id == 'heat'):
            #if heating to setPoint Temp pulse
            if(temp < self.setPoint):
                self.on_heat_pulse()
            #else if it is at temp solid light
            else:
                self.on_enter_heat()

        #if state is COOL
        elif(self.current_state.id == 'cool'):
            #if cooling to setPoint Temp pulse
            if(temp > self.setPoint):
                self.on_cool_pulse()
            #else if it is at temp solid light 
            else:
    ...
    def setupSerialOutput(self):
        #getting temps in current scale
        temp = floor(self.getFahrenheit())
        #configure the output string to be a simple line
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                #setting up the display for line 2 showing the current temp 
                lcd_line_2 = f"T:{self.getFahrenheit():0.1f} F"
                altCounter = altCounter + 1

            else:

                #setting up display line 2 for current state and setPoint
                lcd_line_2 = f"{self.current_state.id} SetT: {self.setPoint} F"
            ...
            if((counter % 30) == 0):
                #use the encode method to turn input into byte array and write it to 
                #serial output
...
#changing the state when the green button is pressed
...
##
redButton = Button(25)
## increase temp when blue button is pressed
...
## decrese temp when blue button is pressed
